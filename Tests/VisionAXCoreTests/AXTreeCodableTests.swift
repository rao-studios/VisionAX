...
@Suite struct AXTreeCodableTests {
    ...
    @Test func theShippedFixtureDecodes() throws {
        ...
        // The engine's role strings (Frigate's `VisionAX.windowRole` and `.regionRole`),
        // spelled out: this module has no engine, and the fixture is the engine's output.
        #expect(window.root?.role == "AXWindow")
        var nodes: [AXNodeSnapshot] = []
        window.root?.forEachNode { nodes.append($0) }
        #expect(nodes.dropFirst().allSatisfy { $0.role == "VXRegion" })
        #expect(window.root?.subtreeCount == nodes.count)
    }
